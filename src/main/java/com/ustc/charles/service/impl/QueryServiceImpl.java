...
public class QueryServiceImpl implements QueryService {
    ...
    @Override
    public List<String> boolQuery(Es es, String title, String author) {
        List<String> blogList = new ArrayList<>();
        SearchResponse response = transportClient.prepareSearch(es.getIndex()).setTypes(es.getType())
                .setQuery(QueryBuilders.boolQuery()
                        .must(QueryBuilders.matchPhraseQuery("title", title))
                        .must(QueryBuilders.wildcardQuery("author", author)))
                .get();
        SearchHits hits = response.getHits();
        for (SearchHit hit : hits) {
            System.out.println(hit.getSourceAsString());
            blogList.add(hit.getSourceAsString());
        }
        return blogList;
    }
